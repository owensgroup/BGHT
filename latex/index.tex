...
We provide a docker image that include the software requirements (except for CUDA drivers). To build the docker image, run\+: {\ttfamily source docker/build} To start the container, run\+: {\ttfamily source docker/run} After starting the container, you can build and execute BGHT code without any additional requirements.\hypertarget{index_autotoc_md10}{}\doxysubsection{limitations}\label{index_autotoc_md10}
