...
struct bchs {
  ...
  using atomic_value_allocator_type =
      typename std::allocator_traits<Allocator>::rebind_alloc<atomic_value_type>;
  using pool_allocator_type =
      typename std::allocator_traits<Allocator>::rebind_alloc<bool>;
  using size_type_allocator_type =
      typename std::allocator_traits<Allocator>::rebind_alloc<size_type>;
   ...
   * @param allocator The allocator to use for allocating GPU device memory
   */
  bchs(std::size_t capacity, Key sentinel_key, Allocator const& allocator = Allocator{});
  /**
   * @brief A shallow-copy constructor
   */
  bchs(const bchs& other);
  /**
   * @brief Move constructor is currently deleted
   */
  bchs(bchs&&) = delete;
  /**
   * @brief The assignment operator for the bchs is currently deleted
   */
  bchs& operator=(const bchs&) = delete;
  /**
   * @brief The move assignment operator for the bchs is currently deleted
   */
  bchs& operator=(bchs&&) = delete;
  /**
   * @brief Destructor that destroys the hash map and deallocate memory if no copies exist
   */
  ~bchs();
  ...
  /**
   * @brief Host-side API for inserting all keys defined by the input argument iterators.
   * All keys in the range must be unique and must not exist in the hash table.
   * @tparam InputIt Device-side iterator that can be converted to `value_type`.
   * @param first An iterator defining the beginning of the input keys to insert
   * @param last  An iterator defining the end of the input keys to insert
   * @param stream  A CUDA stream where the insertion operation will take place
   * @return A boolean indicating success (true) or failure (false) of the insertion
   * operation.
   */
  ...
  /**
   * @brief Device-side cooperative insertion API that inserts a single key into the hash
   * map.
   * @tparam tile_type A cooperative group tile with a size that must match the bucket
   * size of the hash map (i.e., `bucket_size`). It must support the tile-wide intrinsics
   * `ballot`, `shfl`
   * @param key A key-value key to insert into the hash map. The key must be the same
   * for all threads in the  cooperative group tile
   * @param tile  The cooperative group tile
   * @return A boolean indicating success (true) or failure (false) of the insertion
   * operation.
   */
  template <typename tile_type>
  __device__ bool insert(value_type const& key, tile_type const& tile);

  /**
   * @brief Device-side cooperative find API that finds a single key into the hash
   * map.
   * @tparam tile_type A cooperative group tile with a size that must match the bucket
   * size of the hash map (i.e., `bucket_size`). It must support the tile-wide intrinsics
   * `ballot`, `shfl`
   ...
   */
  template <typename tile_type>
  __device__ bool find(key_type const& key, tile_type const& tile);
 ...
 private:
  ...
  mapped_type sentinel_value_{};
  allocator_type allocator_;
  atomic_value_allocator_type atomic_values_allocator_;
  pool_allocator_type pool_allocator_;
  size_type_allocator_type size_type_allocator_;

  atomic_value_type* d_table_{};
  std::shared_ptr<atomic_value_type> table_;
