...
p2bht<Key, T, Hash, KeyEqual, Scope, Allocator, B>::p2bht(std::size_t capacity,
  ...
  table_ =
      std::shared_ptr<atomic_pair_type>(d_table_, bght::hip_deleter<atomic_pair_type>());
...
__device__ bool bght::p2bht<Key, T, Hash, KeyEqual, Scope, Allocator, B>::insert(
  ...
  do {
    ...
    bucket.load(bght::memory_order_relaxed);
    ...
    bucket1.load(bght::memory_order_relaxed);
...
bght::p2bht<Key, T, Hash, KeyEqual, Scope, Allocator, B>::find(key_type const& key,
  ...
  for (int hf = 0; hf < num_hfs; hf++) {
    ...
    cur_bucket.load(bght::memory_order_relaxed);
    INCREMENT_PROBES_IN_TILE
...
p2bht<Key, T, Hash, KeyEqual, Scope, Allocator, B>::size(hipStream_t stream) {
